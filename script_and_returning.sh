...
SET esa_auth_disabled_date = NULL,  
   qr_auth_disabled_date = NULL  
WHERE tenant_id = (  
   SELECT id FROM tenant WHERE host = 'kedo.avmining.ru'  
)  
RETURNING tenant_id, esa_auth_disabled_date, qr_auth_disabled_date;  
\" > result.txt 2>&1"